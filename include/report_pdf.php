...
$sql1 = sprintf($format, $db_days, $_SESSION['user']['user_id'], 
        date('Y-m-d H:i:s', $start), date('Y-m-d H:i:s', $ende));
$kstage = $sqldb->query($sql1);
while ($ks_tag = $kstage->fetch_assoc()) {
    $ks_day = strtotime($ks_tag['pain_day']);
    $ks_art = $ks_tag['ks_art'];
    $sql2 = sprintf("SELECT * FROM `$db_drugdays` WHERE `ks_id` = %s;", 
            $ks_tag['ks_id']);
        ...
        if ($t > date('t', mktime(0, 0, 0, $m, 1, $Y))) {
            ...
            if (isset($tage[$ks_day])) {
                ...
                if (sizeof($ks_data['ks_art']) > 0)
                    $sek[$m] ++;
                $pdf->Cell(3, $height, ($ks_data['ks_art'] & 1) ? 'X' : '', 1, 
                        0, C, $m % 2);
                $pdf->Cell(3, $height, ($ks_data['ks_art'] & 2) ? 'X' : '', 1, 
                        0, C, $m % 2);
                $pdf->Cell(3, $height, ($ks_data['ks_art'] & 4) ? 'X' : '', 1, 
                        0, C, $m % 2);
                $mediliste = '';
                ...
                $pdf->Cell(16, $height, substr($mediliste, 0, - 2), 1, 0, L, 
                        $m % 2);
                for ($g = 0; $g < 5; $g ++)
                    $pdf->Cell(3, $height, 
                            ($ks_data['ks_grad'] == $g) ? 'X' : '', 1, 0, C, 
                            $m % 2);
            } else {
...
if ($showinfo) {
    ...
    foreach ($tage as $tag => $data) {
        ...
        $pdf->Cell(0, 6, 
                utf8_decode(html_entity_decode($data['ks_info'], ENT_QUOTES)), 
                RB);
        $pdf->Ln();
    }
}
