...
if ($_SERVER ['REQUEST_METHOD'] == 'POST') {
		...
		case 'speichern' :
			if (GetParam ( 'button' ) == "Eintrag speichern") {
				...
				if ($fehler)
					...
					if (is_array ( $med )) {
						$values = '';
						foreach ( $med as $med_id )
							$values .= "(NULL , $ks_id, $med_id), ";
	...
	$smarty->assign ( 'ks_arten', $ks_arten );
	$sql = "SELECT * FROM `$stmedis` WHERE med_show = 1 AND med_type = 0 AND (med_user = {$_SESSION['user_id']} OR med_other = 1) ORDER BY med_name ASC";
	$result = $sqldb->query ( $sql );
	$medikamente = array ();
	if ($result->num_rows > 1) {
		while ( $medikament = $result->fetch_assoc () ) {
